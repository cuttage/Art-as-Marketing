...
export default {
  ...
  setup(props) {
    const {
      moreKeyds: vmoreKeyds,
      batchKeyds: vbatchKeyds,
      firstArr: vfirstArr,
    } = useRaw()

    return { vmoreKeyds, vbatchKeyds, vfirstArr }
  },
  data() {
    return {
      diapobKeySub: 'bb' + 2,
  ...
  computed: {
    filteredBlogs() {
      return this.vfirstArr?.value?.body.filter((blog) => {
        return blog.keyword.match(this.search.toLowerCase())
      })
    },
    filteredMoreBlogs() {
      return this.vmoreKeyds.filter((moreblog) => {
    ...
    evalBatch() {
      return !this.vbatchKeyds.includes(this.search.toLowerCase())
    },
    evalSomeA() {
    ...
    search: {
      handler: function (val) {
        if (val.length > 0) {
          ...
          this.hasSomePartial = this.vmoreKeyds.some(function (v) {
            return v.indexOf(val.toLowerCase()) >= 0
          })
          this.hasSomeTotal = this.vbatchKeyds.some(function (v) {
            return v.indexOf(val.toLowerCase()) >= 0
          })
        }
      },
        ...
        this.$nextTick(() => {
          if (this.hasSomeTotal) {
            if (b && s.length === 0) {
              ...
              b &&
              s.length !== 0 &&
              this.vbatchKeyds.includes(s.toLowerCase())
            ) {
              a = b.clientHeight
              c(a)
            }

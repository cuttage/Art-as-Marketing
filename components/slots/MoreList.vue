...
<template>
  ...
  <main class="s-w">
    ...
    <transition-group name="snp" tag="section" class="snp" :style="snp">
      ...
      <menu
        v-for="(options, sfilter) in sft"
        class="sft"
        v-show="menus[sfilter]"
        ref="menu"
        :key="menus[sfilter]"
      >
...
export default {
  ...
  watch: {
    activeMenu(index, from) {
      ...
      this.$nextTick(() => {
        !this.$refs.menu || !this.$refs.menu[index]
          ? (this.snp.height = 0)
          : (this.snp.height = `${this.$refs.menu[index].clientHeight + 16}px`)
      })
    },
    xv: {
  ...
  beforeMount() {
    this.firstArrMore?.value?.body?.forEach(({ keyword }) => {

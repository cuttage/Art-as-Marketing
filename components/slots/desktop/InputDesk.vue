  ...
  <div ref="submenu">
    ...
    <span class="s-b-wrp drpdwn">
      ...
      <div
        ...
        v-show="
          search.length > 0 &&
          search.localeCompare(blog.keyword, undefined, {
            sensitivity: 'accent',
          }) != 0
        "
        @click="setState(blog.keyword)"
      >
    ...
    <span
      v-if="
        search.length > 0 &&
        hasSomePartial &&
        search.localeCompare(batchedVal, undefined, {
          sensitivity: 'accent',
        }) != 0
      "
    >
      ...
      <MoreList :xv="xv">
        ...
        <div
          v-for="moreblog in newExample"
          class="s-b"
          v-show="
            search.length > 0 &&
            search.localeCompare(moreblog, undefined, {
              sensitivity: 'accent',
            }) != 0
          "
          @click="setState(moreblog)"
        >
...
export default {
  ...
  setup(props) {
    const {
      moreKeyds: cmoreKeyds,
      batchKeyds: cbatchKeyds,
      firstArr: cfirstArr,
    } = useRaw()

    return { cmoreKeyds, cbatchKeyds, cfirstArr }
  },
  data() {
    return {
      diapobKeySub: 'bb' + 2,
  ...
  computed: {
    filteredBlogs() {
      return this.cfirstArr?.value?.body?.filter((blog) => {
        return blog.keyword.match(this.search.toLowerCase())
      })
    },
    filteredMoreBlogs() {
      return this.cmoreKeyds.filter((moreblog) => {
    ...
    evalBatchA() {
      return !this.cbatchKeyds.includes(this.search.toLowerCase())
    },
    evalBatchB() {
      return this.cbatchKeyds.includes(this.search.toLowerCase())
    },
    ...
    newExample() {
      ...
      return this.idx < 0
        ? this.filteredMoreBlogs
        : this.filteredMoreBlogs.slice(0, this.idx - 1)
    },
  },
  watch: {
    search: {
      handler: function (val) {
        if (val.length > 0) {
          ...
          this.hasSomePartial = this.cmoreKeyds.some(function (v) {
            return v.indexOf(val.toLowerCase()) >= 0
          })
          this.hasSomeTotal = this.cbatchKeyds.some(function (v) {
            return v.indexOf(val.toLowerCase()) >= 0
          })
        }
      },
    ...
    filteredBoth: {
      handler: function (newVal, oldVal) {
        ...
        let cond2 = (x, y) =>
          x.toString() === y.toString() && s.length === x.toString().length - 1
          ...
          this.$nextTick(() => {
            if (this.hasSomeTotal) {
              if (b && s.length === 0) {
                ...
                (this.caseA && cond1(newVal, oldVal)) ||
                (this.caseA && cond2(newVal, oldVal))
              ) {
                a = b.clientHeight + 94
                c(a)
              } else if (
                (this.caseB && cond1(newVal, oldVal)) ||
                (this.caseB && cond2(newVal, oldVal))
              ) {
                a = b.clientHeight + 16
                c(a)
              } else if (
                b &&
                s.length !== 0 &&
                this.cbatchKeyds.includes(s.toLowerCase())
              ) {
                a = b.clientHeight
                c(a)
              }

...
<<<<<<< HEAD:components/slots/desktop/InputDesk.vue
...
=======
      <div class="absol-head" :key="diapobKeySub" @focusin="muted">
        <input id='search-bar' type="text" autocomplete="off" v-model="search" placeholder="   Browse keywords" ref="inputab" @focusin="muted"/>
      </div>
    </span>
    <span class="single-blog-wrap dropdown">
      <div v-for="blog in filteredBlogs" class="single-blog" v-show="search.length > 0 && search.localeCompare(blog.keyword, undefined, { sensitivity: 'accent' }) != 0" @click="setState(blog.keyword)">
>>>>>>> parent of 494e274... Changed mostly class naming system, intermediate step:components/slots/desktop/InputBlueDesk.vue
        <h2>{{blog.keyword}}</h2>
      </div>
    </span>
    <span v-if="search.length > 0 && hasSomePartial && search.localeCompare(batchedVal, undefined, { sensitivity: 'accent' }) != 0">
      <Snipcart :xv="xv">
        <div v-for="moreblog in newExample" class="single-blog" v-show="search.length > 0 && search.localeCompare(moreblog, undefined, { sensitivity: 'accent' }) != 0" @click="setState(moreblog)">
          <h2>{{moreblog}}</h2>
        </div>
      </Snipcart>
    </span>
...
import Snipcart from '~/components/slots/snipcartBackup.vue'

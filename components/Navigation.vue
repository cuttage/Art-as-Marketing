...
<<<<<<< HEAD:components/Nav.vue
      ...
      <div v-else class="logo" :class="(preventPropertyA && desk) ? 'i5t' : ''" role="button" @click.prevent="giveTime"> <!--:disable only works on button-->
        <slot :name="desk ? 'WrapperSlotA' : 'WrapperSlotB'"></slot>
=======
      <div v-if="portflioCheck" class="logo" :class="(preventPropertyB && desk) ? 'cllogo2' : ''" role="button" @click.prevent="giveTime"> <!--:disable only works on button-->
        <slot :name="desk ? 'WraperSlotA' : 'WraperSlotB'"></slot>
        <slot name="svgContent"></slot>
      </div>
      <div v-else class="logo" :class="(preventPropertyA && desk) ? 'cllogo2' : ''" role="button" @click.prevent="giveTime"> <!--:disable only works on button-->
        <slot :name="desk ? 'WraperSlotA' : 'WraperSlotB'"></slot>
>>>>>>> parent of 494e274... Changed mostly class naming system, intermediate step:components/Navigation.vue
        <slot name="svgContent"></slot>
      </div>
    </header>
  </div>
